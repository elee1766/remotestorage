...
// ToStorageInfo converts the WebFinger record to StorageInfo
func (wf *WebFinger) ToStorageInfo(discoveryURL string) *rs.StorageInfo {
	link := wf.GetRemoteStorageLink()
	if link == nil {
		return nil
	}

	// Convert properties to map[string]interface{} for backward compatibility
	// Try to handle both *rs.RemoteStorageProperties and map[string]any
	var properties map[string]any

	switch props := link.Properties.(type) {
	case *rs.RemoteStorageProperties:
		if props != nil {
			properties = make(map[string]any)

			if props.Version != "" {
				properties["http://remotestorage.io/spec/version"] = props.Version
			}

			if props.AuthEndpoint.Value != nil {
				properties["http://tools.ietf.org/html/rfc6749#section-4.2"] = *props.AuthEndpoint.Value
			}

			if props.QueryTokenSupport.Value != nil {
				properties["http://tools.ietf.org/html/rfc6750#section-2.3"] = *props.QueryTokenSupport.Value
			}

			if props.RangeRequestSupport.Value != nil {
				properties["http://tools.ietf.org/html/rfc7233"] = *props.RangeRequestSupport.Value
			}

			if props.WebAuthoringDomain.Value != nil {
				properties["http://remotestorage.io/spec/web-authoring"] = *props.WebAuthoringDomain.Value
			}
		}
	case map[string]any:
		properties = props
